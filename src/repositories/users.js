...
exports.login = async (_username, _plainTextPassword) => {
    ...
    try {
        ...
        //const result = await bcrypt.compare(_plainTextPassword, hashOficial); //result isboolean
        //const result = true;
        ...
        if (result) {
            ...
            const jwt = jwtLib.sign({ _username }, "minha senha aqui"); // process.env.JWTSECRET
            res.cookie("session", jwt);
            res.status(200).json({ repositories: "GEROU cookie" });
        } else {
